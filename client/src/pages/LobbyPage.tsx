  ...
  return (
    <div className="flex flex-col items-center min-h-screen gap-6 px-4 py-10">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
        <p className="text-bg-3 text-lg uppercase tracking-widest">Room ID</p>
        ...
        <p className="text-bg-3 text-sm mt-1">
          {settings.mode === 'teams' ? 'Teams' : 'Free for all'}
          {' · '}Target: {settings.targetScore}
          {' · '}{settings.bazaarEnabled ? 'Boneyard on' : 'No boneyard'}
        </p>
      </motion.div>
      ...
      <div className="flex gap-2 items-center">
        ...
        />
        <span className="text-bg-3 text-sm">← your name</span>
      </div>

      {settings.mode === 'teams' ? (
        <div className="flex gap-8 w-full max-w-xl">
          <TeamZone label="Team A 🟡" color="teamA" players={teamA}
            onAssign={isHost ? (pid) => send({ type: 'assign_team', playerId: pid, team: 'A' }) : undefined}
          />
          <TeamZone label="Team B 💜" color="teamB" players={teamB}
...
function PlayerCard({ player, isSelf }: { player: Player; isSelf: boolean }) {
  return (
    <div className={`flex items-center gap-3 bg-bg-2 rounded-xl px-4 py-2 ${isSelf ? 'border border-teamA' : ''}`}>
      ...
      <span className="text-lg flex-1">{player.name}{isSelf ? ' (you)' : ''}</span>
      {player.ready && <span className="text-teamA text-sm">✅</span>}
      {!player.connected && <span className="text-danger text-sm">⚡ offline</span>}
    </div>
  );
}
